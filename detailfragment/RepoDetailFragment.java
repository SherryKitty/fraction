...
import com.loqunbai.android.detailfragment.view.RepoDetailHeaderView;
 ...
 * repo详情
 */
public class RepoDetailFragment extends BaseDetailFragment {

    private RepoDetailHeaderView mRepoDetailHeaderView;
    ...
    private AbstractRequestController.AsyncCallback<PostResultModel>
        ...
        public void getResult(PostResultModel result) {
            if (result.status == 0) {
                ...
                ListCacheUpdater.repoCacheUpdater.localUpdateAsync(mDressingItemModel.getId(), mDressingItemModel);
    ...
    private AbstractRequestController.AsyncCallback<PostResultModel>
        ...
        public void getResult(PostResultModel result) {
            if( result.status == 0 ) {
                ...
                DressingCommentListController.getRepoCommentListController().removeCommentById(
    ...
    protected void loadDetail() {
        ListCacheUpdater.repoCacheUpdater.remoteUpdateAsync(
    ...
    protected void initHeaderView() {
        if (isAdded()) {
            mRepoDetailHeaderView = new RepoDetailHeaderView(getActivity(), mListView, mDressingItemModel);
            mListView.addHeaderView(mRepoDetailHeaderView.getView(), null, false);
            setAdapter();
        }
    }
    ...
    protected void init() {
        ...
        TextView tvAppreciate = (TextView) mLick.findViewById(R.id.tv_like);
        if (isAdded() && getActivity() != null) {
            tvAppreciate.setCompoundDrawablesWithIntrinsicBounds(
                    getResources().getDrawable(R.drawable.icon_appreciate_detail_selector),
                    null, null, null
            );
        }
        mLikeCommentFavHelper = new LikeCommentFavHelper(getActivity(),
                mLick, mComment, mFav, LikeCommentFavHelper.ViewType.REPO, mOnActionListener);
    ...
    protected void loadData() {
        ...
        DressingCommentListController.getRepoCommentListController().getNewComments(
    ...
    protected void updateHeaderView( DressingItemModel dressingItemModel) {
        mRepoDetailHeaderView.update(dressingItemModel);
    }

    @Override
    protected int getTitle() {
        return R.string.repo_detail;
    }

    @Override
    protected void updateDetail() {
        ListCacheUpdater.repoCacheUpdater.localUpdateAsync(
                mItemId,
                mDressingItemModel
        );
    }
